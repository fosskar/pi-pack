...
const PATH_FORBIDDEN_CHARACTER =
    ...
    value(...messages) {
      ...
      if (remaining > 0) {
        Array.prototype.push.apply(
          this,
          messages.slice(0, remaining).map(escapeUnsafeDiagnosticCharacters),
        );
      }
      return this.length;
    },
...
function preflightJsonText(contents) {
  ...
  for (let index = 0; index < contents.length; index++) {
    ...
    if (character === '"') {
      ...
      const container = currentContainer();
      if (!container || container.state !== "commaOrEnd")
      ...
      while (
        ...
        !isTokenDelimiter(contents[index + 1])
      )
        index++;
    }
  }
...
function preflightDocument(root) {
  ...
  while (stack.length > 0) {
    ...
    if (depth >= LIMITS.nestingDepth) {
      errors.push(
        `${location}: exceeds nesting depth limit ${LIMITS.nestingDepth}`,
      );
      return errors;
    }
...
function encodeCanonicalRef(value) {
  if (!isCanonicalRef(value))
    throw new TypeError("invalid canonical reference");
  let encoded = "";
...
function canonicalCoverageId(refs) {
  ...
  const fields = hasOwn(refs, "lifecycle")
    ? [...REF_FIELDS, "lifecycle"]
    : REF_FIELDS;
  if (
    Object.keys(refs).length !== fields.length ||
...
function isSafeRelativePath(value) {
  if (
    typeof value !== "string" ||
    value.length === 0 ||
    !hasValidUnicodeScalarValues(value) ||
    value.trim() !== value ||
    PATH_FORBIDDEN_CHARACTER.test(value) ||
    value.includes("\\") ||
    value.includes(":")
...
function validateStringArray(value, location, errors, options = {}) {
  ...
  if (value.length > MAX_LIST_ITEMS)
    errors.push(`${location}: exceeds ${MAX_LIST_ITEMS} entries`);
  const seen = new Set();
  value.slice(0, MAX_LIST_ITEMS).forEach((entry, index) => {
    ...
    if (!valid)
      errors.push(
        `${entryLocation}: invalid ${pathValue ? "repository-relative path" : "text"}`,
      );
    if (
      fingerprint &&
      typeof entry === "string" &&
      !FINGERPRINT_PATTERN.test(entry)
    ) {
      errors.push(`${entryLocation}: invalid fingerprint`);
    }
    if (typeof entry === "string" && seen.has(entry))
      errors.push(`${entryLocation}: duplicate entry`);
  ...
  value.slice(0, MAX_LIST_ITEMS).forEach((check, index) => {
    ...
    for (const field of [
      ...
      if (!hasOwn(check, field))
        errors.push(`${base}: missing required field ${safeQuote(field)}`);
    }
    if (!isSafeAgentId(check.agent_id))
      errors.push(`${base}.agent_id: expected a canonical lowercase agent ID`);
    validateStringArray(
      check.reviewed_paths,
      `${base}.reviewed_paths`,
      errors,
      { allowEmpty: false, pathValue: true },
    );
    if (!isVisibleText(check.invariant))
      errors.push(`${base}.invariant: invalid text`);
    if (check.method !== "source" && check.method !== "local")
      errors.push(`${base}.method: expected "source" or "local"`);
    if (!isVisibleText(check.result))
      errors.push(`${base}.result: invalid text`);
    if (check.method === "source" && check.artifact !== null) {
      ...
      if (!isOwnedArtifactPath(check.artifact, check.agent_id)) {
        errors.push(
          `${base}.artifact: local check requires an artifact owned by agent ${safeQuote(isSafeAgentId(check.agent_id) ? check.agent_id : "<agent-id>")}`,
        );
      }
    } else if (
      check.method !== "source" &&
      check.method !== "local" &&
      check.artifact !== null &&
      !isSafeRelativePath(check.artifact)
    ) {
      errors.push(
        `${base}.artifact: expected null or a safe output-relative path`,
      );
    }
  });
}
...
function validateReviewedPathOwnership(unit, base, errors) {
  if (!Array.isArray(unit.reviewed_paths) || !Array.isArray(unit.local_checks))
    return;
  const aggregatePaths = new Set(
    unit.reviewed_paths.filter((value) => typeof value === "string"),
  );
  ...
  for (const reviewedPath of aggregatePaths) {
    if (!ownedPaths.has(reviewedPath))
      errors.push(
        `${base}.reviewed_paths: ${safeQuote(reviewedPath)} has no check owner`,
      );
  }
  for (const reviewedPath of ownedPaths) {
    if (!aggregatePaths.has(reviewedPath))
      errors.push(
        `${base}.local_checks: owned path ${safeQuote(reviewedPath)} is absent from aggregate reviewed_paths`,
      );
  }
}
...
function validateExcludedBlocks(value, location, errors) {
  ...
  value.slice(0, MAX_LIST_ITEMS).forEach((entry, index) => {
    ...
    if (!isVisibleText(entry.reason))
      errors.push(`${base}.reason: invalid text`);
    if (typeof entry.block === "string" && seen.has(entry.block))
      errors.push(`${base}.block: duplicate entry`);
...
function hasValidSemanticFields(unit) {
  return (
    ["surface", "boundary", "subsystem", "attack_class"].every((field) =>
      isVisibleText(unit[field]),
    ) &&
...
function requireEmptyArray(unit, field, base, errors) {
  if (Array.isArray(unit[field]) && unit[field].length > 0) {
    errors.push(
      `${base}.${field}: unit with status ${safeQuote(unit.status)} must keep this array empty`,
    );
  }
}
...
function requireNonemptyArray(unit, field, base, errors) {
  if (!Array.isArray(unit[field]) || unit[field].length === 0) {
    errors.push(
      `${base}.${field}: unit with status ${safeQuote(unit.status)} requires entries`,
    );
  }
}
...
function validateStateInvariants(unit, base, errors) {
  ...
  const requireOwner = () => {
    if (!isSafeAgentId(unit.agent_id))
      errors.push(
        `${base}.agent_id: unit with status ${safeQuote(unit.status)} requires a canonical lowercase agent ID`,
      );
  };

  if (unit.status !== "candidate")
    requireEmptyArray(unit, "result_fingerprints", base, errors);

  switch (unit.status) {
    case "planned":
      if (unit.agent_id !== null)
        errors.push(`${base}.agent_id: planned unit must be unassigned`);
      emptyEvidence();
    ...
    case "deferred":
      if (unit.agent_id !== null)
        errors.push(
          `${base}.agent_id: unit with status ${safeQuote(unit.status)} must be unassigned`,
        );
      emptyEvidence();
...
function validateAttempts(value, unit, base, errors) {
  ...
  if (value.length > MAX_LIST_ITEMS)
    errors.push(`${base}.attempts: exceeds ${MAX_LIST_ITEMS} entries`);
  ...
  value.slice(0, MAX_LIST_ITEMS).forEach((attempt, index) => {
    ...
    for (const field of ATTEMPT_FIELDS) {
      if (!hasOwn(attempt, field))
        errors.push(
          `${attemptBase}: missing required field ${safeQuote(field)}`,
        );
    }
    if (!Number.isInteger(attempt.wave) || attempt.wave < 1) {
      errors.push(`${attemptBase}.wave: expected a positive integer`);
    } else {
      if (attempt.wave <= previousWave)
        errors.push(
          `${attemptBase}.wave: archived attempt waves must be strictly increasing`,
        );
      if (Number.isInteger(unit.wave) && attempt.wave >= unit.wave) {
        errors.push(
          `${attemptBase}.wave: archived attempt wave must precede current wave ${safeQuote(unit.wave)}`,
        );
      }
    ...
    if (!ATTEMPT_STATUSES.has(attempt.status)) {
      errors.push(
        `${attemptBase}.status: expected "covered", "candidate", or "blocked"`,
      );
    }
    let hasFreshOwner = false;
    if (!isSafeAgentId(attempt.agent_id)) {
      errors.push(
        `${attemptBase}.agent_id: archived attempt requires a canonical lowercase agent ID`,
      );
    } else if (priorOwners.has(attempt.agent_id)) {
      errors.push(
        `${attemptBase}.agent_id: assignment owner must be fresh for each attempt`,
      );
    } else {
      hasFreshOwner = true;
    }
    validateStringArray(
      attempt.reviewed_paths,
      `${attemptBase}.reviewed_paths`,
      errors,
      { pathValue: true },
    );
    ...
    validateStringArray(
      attempt.result_fingerprints,
      `${attemptBase}.result_fingerprints`,
      errors,
      { fingerprint: true },
    );
    validateStringArray(
      attempt.unresolved,
      `${attemptBase}.unresolved`,
      errors,
    );
    if (!isVisibleText(attempt.reassignment_reason))
      errors.push(`${attemptBase}.reassignment_reason: invalid text`);
    ...
    if (Array.isArray(attempt.local_checks)) {
      attempt.local_checks.forEach((check, checkIndex) => {
        ...
        if (priorOwners.has(check.agent_id)) {
          errors.push(
            `${attemptBase}.local_checks[${checkIndex}].agent_id: prior assignment owner evidence must remain in its earlier attempt`,
          );
        }
        if (
          typeof check.artifact === "string" &&
          priorArtifacts.has(check.artifact)
        ) {
          errors.push(
            `${attemptBase}.local_checks[${checkIndex}].artifact: artifact from an earlier attempt cannot be reused`,
          );
        }
        if (check.method === "local" && typeof check.artifact === "string")
          priorArtifacts.add(check.artifact);
      });
    }
  ...
  if (isSafeAgentId(unit.agent_id) && priorOwners.has(unit.agent_id)) {
    errors.push(
      `${base}.agent_id: current assignment owner must be fresh after reassignment`,
    );
  }
  if (Array.isArray(unit.local_checks)) {
    unit.local_checks.forEach((check, index) => {
      ...
      if (priorOwners.has(check.agent_id)) {
        errors.push(
          `${base}.local_checks[${index}].agent_id: prior assignment owner evidence must remain in its archived attempt`,
        );
      }
      if (
        typeof check.artifact === "string" &&
        ...
        errors.push(
          `${base}.local_checks[${index}].artifact: artifact from an archived attempt cannot be reused`,
        );
      }
    });
  }
...
function collectUnitErrors(unit, index) {
  ...
  for (const field of ["surface", "boundary", "subsystem", "attack_class"]) {
    if (!isVisibleText(unit[field]))
      errors.push(`${base}.${field}: invalid text`);
  }
  if (hasOwn(unit, "lifecycle") && !isVisibleText(unit.lifecycle))
    errors.push(`${base}.lifecycle: invalid text`);

  let expectedId = null;
  if (!isObject(unit.canonical_refs)) {
    errors.push(`${base}.canonical_refs: expected object`);
  } else {
    const expectedFields = hasOwn(unit.canonical_refs, "lifecycle")
      ? [...REF_FIELDS, "lifecycle"]
      : REF_FIELDS;
    for (const field of expectedFields) {
      if (!hasOwn(unit.canonical_refs, field)) {
        errors.push(
          `${base}.canonical_refs: missing required field ${safeQuote(field)}`,
        );
      } else if (!isCanonicalRef(unit.canonical_refs[field])) {
        errors.push(
          `${base}.canonical_refs.${field}: invalid canonical reference`,
        );
      }
    }
    if (
      Object.keys(unit.canonical_refs).some(
    ...
    if (
      hasOwn(unit, "lifecycle") !== hasOwn(unit.canonical_refs, "lifecycle")
    ) {
      errors.push(
        `${base}: lifecycle and canonical_refs.lifecycle must appear together`,
      );
    }
    try {
  ...
  if (!isVisibleText(unit.coverage_id, 65536)) {
    ...
    errors.push(
      `${base}.coverage_id: expected canonical ID ${safeQuote(expectedId)}`,
    );
  }

  validateStringArray(unit.starting_paths, `${base}.starting_paths`, errors, {
    allowEmpty: false,
    pathValue: true,
  });
  if (
    unit.ordinary_attack_class_block !== null &&
    !isVisibleText(unit.ordinary_attack_class_block)
  ) {
    errors.push(
      `${base}.ordinary_attack_class_block: expected null or non-empty text`,
    );
  }
  validateStringArray(
    unit.selected_companion_blocks,
    `${base}.selected_companion_blocks`,
    errors,
  );
  validateExcludedBlocks(
    unit.excluded_blocks,
    `${base}.excluded_blocks`,
    errors,
  );
  if (
    Array.isArray(unit.selected_companion_blocks) &&
    Array.isArray(unit.excluded_blocks)
  ) {
    ...
    unit.excluded_blocks.forEach((entry, blockIndex) => {
      if (isObject(entry) && selected.has(entry.block)) {
        errors.push(
          `${base}.excluded_blocks[${blockIndex}].block: block is also selected`,
        );
      }
    });
  }

  if (!PRIOR_STATUSES.has(unit.prior_status))
    errors.push(
      `${base}.prior_status: invalid value ${safeQuote(unit.prior_status)}`,
    );
  if (!STATUSES.has(unit.status))
    errors.push(`${base}.status: invalid value ${safeQuote(unit.status)}`);
  if (!Number.isInteger(unit.wave) || unit.wave < 1)
    errors.push(`${base}.wave: expected a positive integer`);
  if (unit.agent_id !== null && !isSafeAgentId(unit.agent_id))
    errors.push(`${base}.agent_id: expected null or a safe agent ID`);

  validateAttempts(unit.attempts, unit, base, errors);
  validateStringArray(unit.reviewed_paths, `${base}.reviewed_paths`, errors, {
    pathValue: true,
  });
...
function readFileWithinLimit(file) {
  ...
  if (
    ...
    throw new SafeInputError(
      "OS no-follow and nonblocking input protection is unavailable",
    );
  }
  ...
  try {
    ...
    if (stat.size > MAX_INPUT_BYTES)
      throw new SafeInputError(`input exceeds ${MAX_INPUT_BYTES} byte limit`);
...
function validateDocument(ledger) {
  ...
  for (
    let index = 0;
    index < ledger.length && errors.length < LIMITS.validationErrors;
    index++
  ) {
    ...
    if (ids.has(unit.coverage_id)) {
      ...
      const qualifier =
        key !== null && previous.key !== null && previous.key !== key
          ? "canonical identity collision with different semantic fields"
          : "duplicate coverage ID";
      errors.push(
        `$[${index}].coverage_id: ${qualifier} at $[${previous.index}]`,
      );
    } else {
    ...
    if (
      key !== null &&
      semantics.has(key) &&
      semantics.get(key).id !== unit.coverage_id
    ) {
      const previous = semantics.get(key);
      errors.push(
        `$[${index}].canonical_refs: semantic tuple already uses coverage ID ${safeQuote(previous.id)} at $[${previous.index}]`,
      );
    } else if (key !== null) {
    ...
    if (previousId !== null && previousId > unit.coverage_id) {
      errors.push(
        `$[${index}].coverage_id: units must be sorted lexicographically`,
      );
    }
...
function run(file) {
  ...
  try {
    ...
    const reason =
      error instanceof SafeInputError
        ? error.message
        : "input could not be opened or read safely";
  ...
  try {
    ...
    console.error(
      "Failed to validate coverage ledger: unexpected validation error",
    );
    return 1;
  }
  ...
  if (errors.length > 0) {
    const cap =
      errors.length === LIMITS.validationErrors
        ? `; output capped at ${LIMITS.validationErrors}`
        : "";
