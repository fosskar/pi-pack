...
function canonicalKey(value) {
  ...
  function append(chunk) {
    ...
    if (bytes > LIMITS.canonicalKeyBytes) {
      throw new Error(
        `canonical key exceeds ${LIMITS.canonicalKeyBytes} byte limit`,
      );
    }
    chunks.push(chunk);
  }
...
function collectDataLimitErrors(value, location = "$data") {
  ...
  while (stack.length > 0) {
    ...
    if (current.depth > LIMITS.nestingDepth) {
      return [
        escapeUnsafeDiagnosticCharacters(
          `${current.location}: exceeds ${LIMITS.nestingDepth} level nesting depth limit`,
        ),
      ];
    }
    if (seen.has(current.value)) {
      return [
        escapeUnsafeDiagnosticCharacters(
          `${current.location}: input must not contain repeated or cyclic object references`,
        ),
      ];
    }
    ...
    if (Array.isArray(current.value)) {
      if (current.value.length > LIMITS.arrayItems) {
        return [
          escapeUnsafeDiagnosticCharacters(
            `${current.location}: exceeds ${LIMITS.arrayItems} item array limit`,
          ),
        ];
      }
      ...
      for (let index = keys.length - 1; index >= 0; index--) {
        ...
        if (child !== null && typeof child === "object") {
          stack.push({
            value: child,
            location: propertyPath(current.location, key),
            depth: current.depth + 1,
          });
        }
      }
  ...
  function check(node, p) {
    ...
    for (const key of Object.keys(node)) {
      if (!SUPPORTED_KEYWORDS.has(key))
        errors.push(`${p}: unsupported schema keyword ${safeQuote(key)}`);
    }
    ...
    if (hasOwn(node, "properties")) {
      if (
        ...
        for (const key of Object.keys(node.properties))
          check(node.properties[key], propertyPath(`${p}.properties`, key));
      }
    }
    ...
    if (
      ...
      errors.push(
        `${p}.additionalProperties: only boolean values are supported`,
      );
    }
    if (hasOwn(node, "enum")) {
...
function findDiscriminator(schema) {
  if (!hasOwn(schema, "properties") || typeof schema.properties !== "object")
    return null;
...
function validate(value, schema, p, errors) {
  ...
  if (hasOwn(schema, "oneOf")) {
    const results = schema.oneOf.map((branch) =>
      collectUnchecked(value, branch, p),
    );
    ...
    if (passingIndexes.length !== 1) {
      errors.push(
        `${p}: must match exactly one schema in oneOf; matched ${passingIndexes.length}`,
      );
      if (
        passingIndexes.length === 0 &&
        value !== null &&
        typeof value === "object" &&
        !Array.isArray(value)
      ) {
        const matchingDiscriminators = schema.oneOf
          .map((branch, index) => ({
            discriminator: findDiscriminator(branch),
            index,
          }))
          .filter(
            ({ discriminator }) =>
              discriminator &&
              hasOwn(value, discriminator.key) &&
              deepEqual(value[discriminator.key], discriminator.value),
          );
  ...
  if (hasOwn(schema, "const") && !deepEqual(value, schema.const)) {
    errors.push(
      `${p}: must equal ${safeQuote(schema.const)}, got ${safeQuote(value)}`,
    );
  }
  if (
    hasOwn(schema, "enum") &&
    !schema.enum.some((allowed) => deepEqual(value, allowed))
  ) {
    const allowed = schema.enum.map(safeQuote).join(", ");
    errors.push(
      `${p}: invalid value ${safeQuote(value)} (expected one of ${allowed})`,
    );
  }
  ...
  if (typeOf(value) === "object") {
    for (const req of hasOwn(schema, "required") ? schema.required : []) {
      if (!hasOwn(value, req))
        errors.push(`${p}: missing required field ${safeQuote(req)}`);
    }
  ...
  if (Array.isArray(value)) {
    ...
    if (hasOwn(schema, "uniqueItems") && schema.uniqueItems === true) {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        if (setBytes > LIMITS.uniqueSetBytes) {
          errors.push(
            `${p}: canonical uniqueness set exceeds ${LIMITS.uniqueSetBytes} byte limit`,
          );
          break;
        }
    ...
    if (hasOwn(schema, "items")) {
      value.forEach((item, index) =>
        validate(item, schema.items, `${p}[${index}]`, errors),
      );
    }
  }
...
function collectFindingSemanticErrors(findings) {
  ...
  findings.forEach((finding, index) => {
    ...
    if (
      hasOwn(finding, "fingerprint") &&
      typeof finding.fingerprint === "string"
    ) {
      if (fingerprints.has(finding.fingerprint)) {
        errors.push(
          `${base}.fingerprint: duplicate of $[${fingerprints.get(finding.fingerprint)}].fingerprint`,
        );
      } else {
      ...
      if (
        ...
        errors.push(
          `${base}.fingerprint: findings must be sorted lexicographically`,
        );
      }
      previousFingerprint = finding.fingerprint;
    }
    ...
    for (const field of ["trace", "evidence"]) {
      ...
      finding[field].forEach((entry, entryIndex) => {
        ...
        if (
          ...
          errors.push(
            `${base}.${field}[${entryIndex}].line: must be a positive integer`,
          );
        }
        if (hasOwn(entry, "file") && !isSafeRelativeSourcePath(entry.file)) {
          errors.push(
            `${base}.${field}[${entryIndex}].file: must be a safe repository-relative source path`,
          );
        }
      });
    }
    if (
      finding.remediation &&
      Array.isArray(finding.remediation.code_changes)
    ) {
      finding.remediation.code_changes.forEach((change, changeIndex) => {
        if (
          change &&
          hasOwn(change, "file_name") &&
          !isSafeRelativeSourcePath(change.file_name)
        ) {
          errors.push(
            `${base}.remediation.code_changes[${changeIndex}].file_name: must be a safe repository-relative source path`,
          );
        }
      });
    }
    ...
    if (Array.isArray(finding.trace) && finding.trace.length === 1) {
      ...
      if (kind !== "entrypoint" && kind !== "sink") {
        errors.push(
          `${base}.trace[0].kind: a one-line trace must be "entrypoint" or "sink"`,
        );
      }
      ...
      if (finding.trace[0] && finding.trace[0].kind !== "entrypoint") {
        errors.push(
          `${base}.trace[0].kind: must be "entrypoint", got ${safeQuote(finding.trace[0].kind)}`,
        );
      }
      if (finding.trace[last] && finding.trace[last].kind !== "sink") {
        errors.push(
          `${base}.trace[${last}].kind: must be "sink", got ${safeQuote(finding.trace[last].kind)}`,
        );
      }
      for (let traceIndex = 1; traceIndex < last; traceIndex++) {
        if (
          finding.trace[traceIndex] &&
          finding.trace[traceIndex].kind !== "propagation"
        ) {
          errors.push(
            `${base}.trace[${traceIndex}].kind: must be "propagation", got ${safeQuote(finding.trace[traceIndex].kind)}`,
          );
        }
      }
    }
    ...
    if (verdict === "confirmed") {
      for (const forbidden of [
        ...
        if (hasOwn(finding, forbidden))
          errors.push(
            `${base}: confirmed finding must not contain ${safeQuote(forbidden)}`,
          );
      }
      if (
        !finding.execution ||
        typeof finding.execution !== "object" ||
        typeof finding.execution.observed_result !== "string" ||
        !hasVisibleProse(finding.execution.observed_result)
      ) {
        errors.push(
          `${base}: confirmed finding requires a visible execution observed_result`,
        );
      }
      if (
        !finding.remediation ||
        typeof finding.remediation !== "object" ||
        typeof finding.remediation.strategy !== "string" ||
        !hasVisibleProse(finding.remediation.strategy)
      ) {
      ...
      const impact =
        ...
        finding.severity.impact.score;
      if (
        SEVERITY_RANK.has(overall) &&
        SEVERITY_RANK.has(impact) &&
        SEVERITY_RANK.get(overall) > SEVERITY_RANK.get(impact)
      ) {
        errors.push(
          `${base}.severity.overall_severity: cannot exceed demonstrated impact ${safeQuote(impact)}`,
        );
      }
    } else if (verdict === "needs_validation") {
      if (hasOwn(finding, "severity"))
        errors.push(
          `${base}: needs_validation finding must not contain "severity"`,
        );
      for (const forbidden of [
        "execution",
        "remediation",
        ...
        if (hasOwn(finding, forbidden))
          errors.push(
            `${base}: needs_validation finding must not contain ${safeQuote(forbidden)}`,
          );
      }
      const plan = finding.validation_plan;
      const hasLocalPlan =
        plan && typeof plan.local === "string" && hasVisibleProse(plan.local);
      const hasDeploymentPlan =
        plan &&
        typeof plan.deployment === "string" &&
        hasVisibleProse(plan.deployment);
      if (!hasLocalPlan && !hasDeploymentPlan) {
        errors.push(
          `${base}.validation_plan: requires at least one visible local or deployment plan`,
        );
      }
      ...
      for (const forbidden of [
        ...
        if (hasOwn(finding, forbidden))
          errors.push(
            `${base}: rejected finding must not contain ${safeQuote(forbidden)}`,
          );
      }
    }
  });
...
function loadSchema(schemaPath) {
  ...
  if (errors.length > 0)
    throw new Error(
      `unsupported or invalid report schema:\n${errors.join("\n")}`,
    );
  return schema;
}
  ...
  try {
    ...
    while (true) {
      ...
      if (bytesRead > LIMITS.inputBytes) {
        throw new SafeInputError(
          `input exceeds ${LIMITS.inputBytes} byte limit`,
        );
      }
...
function enforceJsonTextLimits(contents) {
  ...
  function markArrayItem() {
    ...
    if (!container || container.type !== "array" || !container.expectsItem)
      return;
    container.expectsItem = false;
    container.items++;
    if (container.items > LIMITS.arrayItems) {
      throw new JsonStructureError(
        `input exceeds ${LIMITS.arrayItems} item array limit`,
      );
    }
  }
  ...
  for (let index = 0; index < contents.length; index++) {
    ...
    if (character === '"') {
      ...
      if (containers.length >= LIMITS.nestingDepth) {
        throw new JsonStructureError(
          `input exceeds ${LIMITS.nestingDepth} level nesting depth limit`,
        );
      }
      containers.push({
...
function run(file) {
  ...
  try {
    ...
    console.error(
      "Failed to validate findings JSON: unexpected validation error",
    );
    return 1;
  }
  for (const message of errors)
    console.error("ERROR:", escapeUnsafeDiagnosticCharacters(message));
  if (errors.length > 0) {
    const cap =
